...
- (BOOL)_writeHTTPResponseWithStatusCode:(NSInteger)statusCode image:(UIImage *)image {
    ...
    if (data) {
        ...
        TLog_ERROR(@"Failed serializing HTTP response");
    }
    CFRelease(response);
    ...
    if ([method isEqualToString:@"GET"]) {
        ...
        if ([path isEqualToString:@"/"]) {
            ...
            if([icon isKindOfClass:[UIImage class]]) {
                ...
                TLog_WARNING(@"Unsupported path in HTTP request: %@", path);
        ...
        TLog_WARNING(@"Unsupported method in HTTP request: %@", method);
        ...
        if (data) {
            ...
            if (range.location != NSNotFound) {
                ...
                if (CFHTTPMessageIsHeaderComplete(message)) {
                    ...
                    TLog_ERROR(@"Failed parsing HTTP request headers");
                }
                CFRelease(message);
                if (!success) {
                    [self close];
...
@interface HJTraceLoggerServer() {
    dispatch_semaphore_t _pollingSemaphore;
}
@property (nonatomic, assign) NSTimeInterval refreshDelay;
@end

@implementation HJTraceLoggerServer
